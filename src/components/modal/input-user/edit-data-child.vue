...
import { useUserPutChild } from '@/services/child'
...
const { mutate, isPending } = useUserPutChild(
  computed(() => props.id)
)
...
const submitForm = () => {
  formRef.value?.validate((errors) => { console.log(errors)
    if (!errors) {
      mutate({
        ...formData.value,
  ...
  <div class="flex items-center justify-center">
    <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-2xl">
      ...
      <n-form ref="formRef" :model="formData" @submit.prevent="submitForm">
        ...
        <!-- Mobile View -->
        <div class="grid grid-cols-2 gap-4 mb-4 mx-4 md:hidden">
          <div>
            <n-form-item label="Tempat Lahir" path="TTL">
              <n-select v-model:value="formData.placeOfBirth" :options="placeOfBirthOptions" size="large" />
            </n-form-item>
      ...
      </n-form> <!-- End tag n-form ditambahkan di sini -->
    </div>
  </div>
</template>
